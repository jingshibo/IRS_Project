...
def _reorder_grouped_matrix(
    ...
    for liquid in liquids:
        ...
        reordered_liquid_to_indices[liquid] = list(range(next_index, next_index + len(original_indices)))
...
def summarize_independent_models_results(fold_results: List[object]) -> Dict[str, object]:
...
def print_independent_models_summary(summary: Dict[str, object]) -> None:
    print(
        f"Independent models measurement joint acc={summary['mean_measurement_joint_acc']:.4f} "
        f"+/- {summary['std_measurement_joint_acc']:.4f}"
    )
    print(
        f"Independent models sample joint acc={summary['mean_sample_joint_acc']:.4f} "
        f"+/- {summary['std_sample_joint_acc']:.4f}"
    )
    print(
        f"Independent models measurement liquid acc={summary['mean_measurement_liquid_acc']:.4f} "
        f"+/- {summary['std_measurement_liquid_acc']:.4f}"
    )
    print(
        f"Independent models sample liquid acc={summary['mean_sample_liquid_acc']:.4f} "
        f"+/- {summary['std_sample_liquid_acc']:.4f}"
    )
    print(
        f"Independent models measurement concentration-group acc={summary['mean_measurement_concentration_acc']:.4f} "
        f"+/- {summary['std_measurement_concentration_acc']:.4f}"
    )
    print(
        f"Independent models sample concentration-group acc={summary['mean_sample_concentration_acc']:.4f} "
...
def plot_grouped_recall_matrix_with_numbers(
    ...
    *,
    ...
    ax.set_title(title or "Independent Models Test Recall Matrix")
