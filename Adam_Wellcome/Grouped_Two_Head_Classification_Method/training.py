...
from .model import GroupedClassificationAdamWellcomeCNN1D
...
class SignalDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        return self.x[index], self.liquid_y[index], self.concentration_y[index]


@dataclass
class GroupedClassificationConfig:
    epochs: int = 100
    batch_size: int = 32
    lr: float = 1e-3
    weight_decay: float = 1e-2
    liquid_label_smoothing: float = 0.05
    concentration_label_smoothing: float = 0.0
...
@dataclass
class GroupedClassificationFoldResult:
    fold: int
    model: nn.Module
...
def _split_joint_label(label: str) -> tuple[str, str]:
    if "__" not in label:
        raise ValueError(
            "Grouped classification training requires joint labels formatted as 'liquid__concentration_group'."
        )
...
class GroupedClassificationTrainer:
    def __init__(self, config: Optional[GroupedClassificationConfig] = None):
        self.config = config or GroupedClassificationConfig()
    ...
    def _run_train_epoch(
        ...
        concentration_criterion: nn.Module,
        optimizer: torch.optim.Optimizer,
        ...
        for x_batch, liquid_y, concentration_y in loader:
            ...
            concentration_loss = concentration_criterion(concentration_logits, concentration_y)
            loss = liquid_loss + self.config.concentration_loss_weight * concentration_loss
            loss.backward()
            optimizer.step()

            liquid_pred = torch.argmax(liquid_logits, dim=1)
            concentration_pred = torch.argmax(concentration_logits, dim=1)
    ...
    def _predict_outputs(
        ...
        for x_batch, _, _ in loader:
            liquid_logits, concentration_logits = model(x_batch.to(self.device))
            liquid_probabilities.append(torch.softmax(liquid_logits, dim=1).cpu().numpy())
            concentration_probabilities.append(torch.softmax(concentration_logits, dim=1).cpu().numpy())
        return (
            np.concatenate(liquid_probabilities).astype(np.float32, copy=False),
            np.concatenate(concentration_probabilities).astype(np.float32, copy=False),
        )
    ...
    def fit(self, split: RepeatedMeasurementSplit) -> GroupedClassificationFoldResult:
        ...
        train_loader = DataLoader(
            SignalDataset(split.x_train, y_train_liquid, y_train_concentration),
            batch_size=self.config.batch_size,
            shuffle=True,
            num_workers=self.config.num_workers,
        )

        model = GroupedClassificationAdamWellcomeCNN1D(
        ...
        concentration_criterion = nn.CrossEntropyLoss(
        ...
        return GroupedClassificationFoldResult(
            fold=split.fold,
            model=model,
            history=history,
            measurement_joint_acc=measurement_joint_acc,
    ...
    def _evaluate_predictions(
        ...
        *,
        ...
        liquid_probabilities, concentration_probabilities = self._predict_outputs(model, x_test)
        liquid_pred_idx = np.argmax(liquid_probabilities, axis=1)
        concentration_pred_idx = np.argmax(concentration_probabilities, axis=1)
        ...
        for group_id in sorted(set(int(value) for value in test_group_ids)):
            ...
            mean_concentration_probability = concentration_probabilities[mask].mean(axis=0)
            concentration_idx = int(np.argmax(mean_concentration_probability))
