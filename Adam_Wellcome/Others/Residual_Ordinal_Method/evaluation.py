...
def summarize_residual_ordinal_results(fold_results: List[object]) -> Dict[str, object]:
...
def print_residual_ordinal_summary(summary: Dict[str, object]) -> None:
    print(
        f"Residual ordinal measurement joint acc={summary['mean_measurement_joint_acc']:.4f} "
        f"+/- {summary['std_measurement_joint_acc']:.4f}"
    )
    print(
        f"Residual ordinal sample joint acc={summary['mean_sample_joint_acc']:.4f} "
        f"+/- {summary['std_sample_joint_acc']:.4f}"
    )
    print(
        f"Residual ordinal measurement liquid acc={summary['mean_measurement_liquid_acc']:.4f} "
        f"+/- {summary['std_measurement_liquid_acc']:.4f}"
    )
    print(
        f"Residual ordinal sample liquid acc={summary['mean_sample_liquid_acc']:.4f} "
        f"+/- {summary['std_sample_liquid_acc']:.4f}"
    )
    print(
        f"Residual ordinal measurement concentration acc={summary['mean_measurement_concentration_acc']:.4f} "
        f"+/- {summary['std_measurement_concentration_acc']:.4f}"
    )
    print(
        f"Residual ordinal sample concentration acc={summary['mean_sample_concentration_acc']:.4f} "
...
def plot_grouped_recall_matrix(
    ...
    *,
    ...
    ax.set_title(title or f"Residual Ordinal {matrix_name.title()} Recall Matrix")
