...
from .model import ResidualOrdinalAdamWellcomeCNN1D
from .preprocessing import build_liquid_reference_residuals
...
class LiquidSignalDataset(Dataset):
...
@dataclass
class ResidualOrdinalFoldResult:
    fold: int
    model: nn.Module
    ...
    liquid_reference_means: Dict[str, np.ndarray]
    preprocessing_test_liquids: List[str]


def _split_joint_label(label: str) -> tuple[str, str]:
    if "__" not in label:
        raise ValueError(
            "Residual ordinal training requires joint labels formatted as 'liquid__concentration'."
        )
...
class ResidualOrdinalTrainer:
    def __init__(self, config: Optional[ResidualOrdinalConfig] = None):
        self.config = config or ResidualOrdinalConfig()
    ...
    @torch.no_grad()
    def _predict_liquid_probabilities(
        self,
        model: nn.Module,
        x: np.ndarray,
    ) -> np.ndarray:
        model.eval()
        loader = DataLoader(
            LiquidSignalDataset(x, np.zeros(len(x), dtype=np.int64)),
            batch_size=self.config.batch_size,
            shuffle=False,
            num_workers=self.config.num_workers,
        )
    ...
    def _fit_liquid_preprocessor(
        ...
        *,
        ...
        train_loader = DataLoader(
            LiquidSignalDataset(x_train, y_train),
            batch_size=self.config.batch_size,
            shuffle=True,
            num_workers=self.config.num_workers,
        )
        ...
        for epoch_idx in range(self.config.epochs):
            self._run_liquid_train_epoch(model, train_loader, criterion, optimizer)
            if scheduler is not None:
                scheduler.step()

            stop_probabilities = self._predict_liquid_probabilities(model, x_stop)
            stop_predictions = np.argmax(stop_probabilities, axis=1)
            stop_acc = float((stop_predictions == y_stop).mean())
    ...
    def fit(self, split: RepeatedMeasurementSplit) -> ResidualOrdinalFoldResult:
        ...
        liquid_preprocessor = self._fit_liquid_preprocessor(
            x_train=split.x_train,
            y_train=y_train_liquid,
            x_stop=split.x_test,
            y_stop=y_test_liquid,
        )
        preprocessing_test_liquid_idx = np.argmax(
            self._predict_liquid_probabilities(liquid_preprocessor, split.x_test),
            axis=1,
        )
        preprocessing_test_liquids = [
            idx_to_liquid_label[int(idx)] for idx in preprocessing_test_liquid_idx
        ]

        preprocessed = build_liquid_reference_residuals(
            x_train=split.x_train,
            x_test=split.x_test,
            train_liquids=train_liquids,
            test_liquids=preprocessing_test_liquids,
            residual_mode=self.config.residual_mode,
        )
        x_train_model = preprocessed.x_train
        x_test_model = preprocessed.x_test
        ...
        model = ResidualOrdinalAdamWellcomeCNN1D(
            in_channels=int(x_train_model.shape[1]),
        ...
        return ResidualOrdinalFoldResult(
            fold=split.fold,
            model=model,
            history=history,
            measurement_joint_acc=measurement_joint_acc,
            ...
            concentration_label_to_idx=concentration_label_to_idx,
            idx_to_concentration_label=idx_to_concentration_label,
            liquid_reference_means=preprocessed.liquid_means,
            preprocessing_test_liquids=preprocessing_test_liquids,
        )
