...
def summarize_ordinal_50_class_results(fold_results: List[object]) -> Dict[str, object]:
...
def print_ordinal_50_class_summary(summary: Dict[str, object]) -> None:
    print(
        f"Ordinal-50 measurement joint acc={summary['mean_measurement_joint_acc']:.4f} "
        f"+/- {summary['std_measurement_joint_acc']:.4f}"
    )
    print(
        f"Ordinal-50 sample joint acc={summary['mean_sample_joint_acc']:.4f} "
        f"+/- {summary['std_sample_joint_acc']:.4f}"
    )
    print(
        f"Derived measurement liquid acc={summary['mean_measurement_liquid_acc']:.4f} "
        f"+/- {summary['std_measurement_liquid_acc']:.4f}"
    )
    print(
        f"Derived sample liquid acc={summary['mean_sample_liquid_acc']:.4f} "
        f"+/- {summary['std_sample_liquid_acc']:.4f}"
    )
    print(
        f"Derived measurement concentration acc={summary['mean_measurement_concentration_acc']:.4f} "
        f"+/- {summary['std_measurement_concentration_acc']:.4f}"
    )
    print(
        f"Derived sample concentration acc={summary['mean_sample_concentration_acc']:.4f} "
...
def plot_grouped_recall_matrix(
    ...
    *,
    ...
    ax.set_title(title or f"Ordinal-50 {matrix_name.title()} Recall Matrix")
