...
def summarize_two_stage_results(fold_results: List[object]) -> Dict[str, object]:
    measurement_joint_accs = np.asarray(
        [result.measurement_joint_acc for result in fold_results],
        dtype=np.float64,
    )
    sample_joint_accs = np.asarray(
        [result.sample_joint_acc for result in fold_results],
        dtype=np.float64,
    )
    measurement_liquid_accs = np.asarray(
        [result.measurement_liquid_acc for result in fold_results],
        dtype=np.float64,
    )
    sample_liquid_accs = np.asarray(
        [result.sample_liquid_acc for result in fold_results],
        dtype=np.float64,
    )
...
def print_two_stage_summary(summary: Dict[str, object]) -> None:
    print(
        f"Two-stage measurement joint acc={summary['mean_measurement_joint_acc']:.4f} "
        f"+/- {summary['std_measurement_joint_acc']:.4f}"
    )
    print(
        f"Two-stage sample joint acc={summary['mean_sample_joint_acc']:.4f} "
        f"+/- {summary['std_sample_joint_acc']:.4f}"
    )
    print(
        f"Stage-1 measurement liquid acc={summary['mean_measurement_liquid_acc']:.4f} "
        f"+/- {summary['std_measurement_liquid_acc']:.4f}"
    )
    print(
        f"Stage-1 sample liquid acc={summary['mean_sample_liquid_acc']:.4f} "
...
def plot_grouped_recall_matrix(
    ...
    *,
    ...
    ax.set_title(title or f"Two-Stage {matrix_name.title()} Recall Matrix")
