...
def summarize_two_stage_ordinal_results(fold_results: List[object]) -> Dict[str, object]:
...
def print_two_stage_ordinal_summary(summary: Dict[str, object]) -> None:
    print(
        f"Two-stage ordinal measurement joint acc={summary['mean_measurement_joint_acc']:.4f} "
        f"+/- {summary['std_measurement_joint_acc']:.4f}"
    )
    print(
        f"Two-stage ordinal sample joint acc={summary['mean_sample_joint_acc']:.4f} "
...
def plot_grouped_recall_matrix(
    ...
    *,
    ...
    ax.set_title(title or f"Two-Stage Ordinal {matrix_name.title()} Recall Matrix")
