...
@dataclass
class TwoStageOrdinalFoldResult:
    fold: int
    liquid_classifier: ClassifierResult
    concentration_classifiers: Dict[str, OrdinalClassifierResult]
...
def _split_joint_label(label: str) -> tuple[str, str]:
    if "__" not in label:
        raise ValueError(
            "Two-stage ordinal training requires joint labels formatted as 'liquid__concentration'."
        )
...
class TwoStageOrdinalTrainer:
    def __init__(self, config: Optional[TwoStageOrdinalConfig] = None):
        self.config = config or TwoStageOrdinalConfig()
    ...
    def _fit_ordinal_classifier(
        ...
        *,
        ...
        train_loader = DataLoader(
            OrdinalSignalDataset(x_train, y_train, ordinal_train_targets),
            batch_size=self.config.concentration_batch_size,
            shuffle=True,
            num_workers=self.config.num_workers,
        )
        ...
        for epoch_idx in range(self.config.epochs):
            ...
            if scheduler is not None:
                scheduler.step()
            stop_logits = self._predict_ordinal_logits(model, x_stop)
        ...
        return OrdinalClassifierResult(model=model, label_to_idx=label_to_idx, idx_to_label=idx_to_label, history=history)
    ...
    def fit(self, split: RepeatedMeasurementSplit) -> TwoStageOrdinalFoldResult:
        ...
        concentration_logits_by_liquid = {
            liquid: self._predict_ordinal_logits(result.model, split.x_test)
        ...
        for row_idx, liquid in enumerate(liquid_pred_labels):
            ...
            concentration_idx = int(self._decode_ordinal(concentration_logits_by_liquid[liquid][row_idx:row_idx + 1])[0])
        ...
        for group_id in sorted(set(int(value) for value in split.test_group_ids)):
            ...
            concentration_result = concentration_classifiers[liquid]
            mean_concentration_logit = concentration_logits_by_liquid[liquid][mask].mean(axis=0, keepdims=True)
        ...
        return TwoStageOrdinalFoldResult(
            fold=split.fold,
