...
class RegressionAdamWellcomeCNN1D(nn.Module):
    def __init__(
        self,
        *,
        in_channels: int,
        ...
        self.concentration_head = nn.Linear(classifier_hidden, 1)

    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        x = self.features(x)
        x = self.embedding(x)
        return self.liquid_head(x), self.concentration_head(x).squeeze(1)
