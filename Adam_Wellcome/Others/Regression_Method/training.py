...
from .model import RegressionAdamWellcomeCNN1D
...
class SignalDataset(Dataset):
    def __init__(
        ...
        self.concentration_target = torch.as_tensor(concentration_target, dtype=torch.float32)
    ...
    def __getitem__(self, index: int):
        return (
            ...
            self.concentration_target[index],
        )
...
class RegressionFoldResult:
    ...
    measurement_concentration_mae: float
    sample_concentration_mae: float
...
def _split_joint_label(label: str) -> tuple[str, str]:
    if "__" not in label:
        raise ValueError(
            "Regression training requires joint labels formatted as 'liquid__concentration'."
        )
...
def _scale_concentration_targets(y: np.ndarray, num_classes: int) -> np.ndarray:
    if num_classes < 2:
        raise ValueError("Regression concentration targets require at least 2 classes")
    return (y.astype(np.float32, copy=False) / float(num_classes - 1)).astype(np.float32, copy=False)
...
class RegressionTrainer:
    ...
    def _run_train_epoch(
        ...
        for x_batch, liquid_y, concentration_y, concentration_target in loader:
            x_batch = x_batch.to(self.device)
            liquid_y = liquid_y.to(self.device)
            concentration_y = concentration_y.to(self.device)
            concentration_target = concentration_target.to(self.device)

            optimizer.zero_grad(set_to_none=True)
            liquid_logits, concentration_value = model(x_batch)
            liquid_loss = liquid_criterion(liquid_logits, liquid_y)
            regression_loss = regression_criterion(concentration_value, concentration_target)
            loss = liquid_loss + self.config.concentration_loss_weight * regression_loss
            loss.backward()
            optimizer.step()

            liquid_pred = torch.argmax(liquid_logits, dim=1)
            concentration_pred = torch.round(concentration_value * float(num_concentrations - 1)).long()
            concentration_pred = torch.clamp(concentration_pred, min=0, max=num_concentrations - 1)
        ...
        return (
            ...
            total_regression_loss / max(total_items, 1),
    ...
    def _predict_outputs(
        ...
        for x_batch, _, _, _ in loader:
            liquid_logits, concentration_value = model(x_batch.to(self.device))
            liquid_probabilities.append(torch.softmax(liquid_logits, dim=1).cpu().numpy())
            concentration_values.append(concentration_value.cpu().numpy())
        return (
            np.concatenate(liquid_probabilities).astype(np.float32, copy=False),
            np.concatenate(concentration_values).astype(np.float32, copy=False),
        )

    @staticmethod
    def _decode_concentration(concentration_values: np.ndarray, num_classes: int) -> np.ndarray:
        scaled = concentration_values * float(num_classes - 1)
        rounded = np.rint(scaled)
        return np.clip(rounded, 0, num_classes - 1).astype(np.int64, copy=False)
    ...
    def fit(self, split: RepeatedMeasurementSplit) -> RegressionFoldResult:
        ...
        train_concentration_targets = _scale_concentration_targets(
        ...
        model = RegressionAdamWellcomeCNN1D(
        ...
        for epoch_idx in range(self.config.epochs):
            train_loss, train_regression_loss, train_liquid_acc, train_concentration_acc = self._run_train_epoch(
                model,
                train_loader,
                liquid_criterion,
            ...
            history["train_regression_loss"].append(train_regression_loss)
            ...
            if self.config.verbose:
                ...
                print(
                    ...
                    f"train_reg_loss={train_regression_loss:.4f} "
        ...
        (
            ...
            measurement_concentration_acc,
            sample_concentration_acc,
            measurement_concentration_mae,
            sample_concentration_mae,
            y_measurement_pred,
            y_sample_true,
            y_sample_pred,
        ...
        return RegressionFoldResult(
            ...
            measurement_concentration_acc=measurement_concentration_acc,
            sample_concentration_acc=sample_concentration_acc,
            measurement_concentration_mae=measurement_concentration_mae,
            sample_concentration_mae=sample_concentration_mae,
    ...
    def _evaluate_predictions(
        ...
        *,
        ...
        idx_to_concentration_label: Dict[int, str],
        joint_label_to_idx: Dict[str, int],
    ) -> tuple[float, float, float, float, float, float, float, float, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        liquid_probabilities, concentration_values = self._predict_outputs(model, x_test)
        liquid_pred_idx = np.argmax(liquid_probabilities, axis=1)
        concentration_pred_idx = self._decode_concentration(
            concentration_values,
            num_classes=len(concentration_label_to_idx),
        )
        ...
        measurement_concentration_mae = float(np.abs(concentration_values * (len(concentration_label_to_idx) - 1) - y_test_concentration).mean())
        ...
        sample_concentration_value: List[float] = []
        ...
        for group_id in sorted(set(int(value) for value in test_group_ids)):
            ...
            concentration_value = float(concentration_values[mask].mean())
            concentration_idx = int(self._decode_concentration(
                np.asarray([concentration_value], dtype=np.float32),
                num_classes=len(concentration_label_to_idx),
            )[0])
            ...
            sample_concentration_value.append(concentration_value)
        ...
        sample_concentration_true_array = np.asarray(sample_concentration_true, dtype=np.int64)
        sample_concentration_pred_array = np.asarray(sample_concentration_pred, dtype=np.int64)
        sample_concentration_value_array = np.asarray(sample_concentration_value, dtype=np.float32)
        sample_concentration_acc = float((sample_concentration_true_array == sample_concentration_pred_array).mean())
        sample_concentration_mae = float(
            np.abs(sample_concentration_value_array * (len(concentration_label_to_idx) - 1) - sample_concentration_true_array).mean()
        )
