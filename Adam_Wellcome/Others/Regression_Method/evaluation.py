...
def summarize_regression_results(fold_results: List[object]) -> Dict[str, object]:
    ...
    measurement_concentration_maes = np.asarray(
        [result.measurement_concentration_mae for result in fold_results],
        dtype=np.float64,
    )
    sample_concentration_maes = np.asarray(
        [result.sample_concentration_mae for result in fold_results],
        dtype=np.float64,
    )
    ...
    return {
        ...
        "mean_measurement_concentration_mae": float(np.mean(measurement_concentration_maes)),
        "std_measurement_concentration_mae": float(np.std(measurement_concentration_maes)),
        "mean_sample_concentration_mae": float(np.mean(sample_concentration_maes)),
        "std_sample_concentration_mae": float(np.std(sample_concentration_maes)),
...
def print_regression_summary(summary: Dict[str, object]) -> None:
    print(
        f"Regression measurement joint acc={summary['mean_measurement_joint_acc']:.4f} "
        f"+/- {summary['std_measurement_joint_acc']:.4f}"
    )
    print(
        f"Regression sample joint acc={summary['mean_sample_joint_acc']:.4f} "
        f"+/- {summary['std_sample_joint_acc']:.4f}"
    )
    print(
        f"Regression measurement liquid acc={summary['mean_measurement_liquid_acc']:.4f} "
        f"+/- {summary['std_measurement_liquid_acc']:.4f}"
    )
    print(
        f"Regression sample liquid acc={summary['mean_sample_liquid_acc']:.4f} "
        f"+/- {summary['std_sample_liquid_acc']:.4f}"
    )
    print(
        f"Regression measurement concentration acc={summary['mean_measurement_concentration_acc']:.4f} "
        f"+/- {summary['std_measurement_concentration_acc']:.4f}"
    )
    print(
        f"Regression sample concentration acc={summary['mean_sample_concentration_acc']:.4f} "
        f"+/- {summary['std_sample_concentration_acc']:.4f}"
    )
    print(
        f"Regression measurement concentration MAE={summary['mean_measurement_concentration_mae']:.4f} "
        f"+/- {summary['std_measurement_concentration_mae']:.4f} classes"
    )
    print(
        f"Regression sample concentration MAE={summary['mean_sample_concentration_mae']:.4f} "
        f"+/- {summary['std_sample_concentration_mae']:.4f} classes"
    )
...
def plot_grouped_recall_matrix(
    ...
    *,
    ...
    ax.set_title(title or f"Regression {matrix_name.title()} Recall Matrix")
