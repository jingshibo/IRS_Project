...
from .model import GroupedJointClassificationCNN1D
...
@dataclass
class GroupedJointClassificationConfig:
    epochs: int = 100
    batch_size: int = 32
    lr: float = 1e-3
...
@dataclass
class GroupedJointClassificationFoldResult:
    fold: int
    model: nn.Module
...
def _split_joint_label(label: str) -> tuple[str, str]:
    if "__" not in label:
        raise ValueError("Joint labels must be formatted as 'liquid__concentration_group'.")
...
class GroupedJointClassificationTrainer:
    def __init__(self, config: Optional[GroupedJointClassificationConfig] = None):
        self.config = config or GroupedJointClassificationConfig()
    ...
    def _labels_from_keys(self, keys: List[dict]) -> List[str]:
        labels = []
        for key in keys:
            concentration_group = self._group_concentration_label(key["concentration"])
            labels.append(f"{key['liquid']}__{concentration_group}")
        return labels
    ...
    def _split_component_arrays(
        ...
        concentrations = []
        for idx in y:
            liquid, concentration = _split_joint_label(idx_to_joint_label[int(idx)])
            liquids.append(liquid)
            concentrations.append(concentration)
        return np.asarray(liquids), np.asarray(concentrations)
    ...
    def _evaluate_predictions(
        ...
        *,
        ...
        y_measurement_pred = np.argmax(probabilities, axis=1).astype(np.int64, copy=False)

        measurement_joint_acc = float((y_test == y_measurement_pred).mean())
        true_liquids, true_concentrations = self._split_component_arrays(y_test, idx_to_joint_label)
        pred_liquids, pred_concentrations = self._split_component_arrays(y_measurement_pred, idx_to_joint_label)
        measurement_liquid_acc = float((true_liquids == pred_liquids).mean())
        measurement_concentration_acc = float((true_concentrations == pred_concentrations).mean())
        ...
        for group_id in sorted(set(int(value) for value in test_group_ids)):
            ...
            sample_pred.append(int(np.argmax(mean_probability)))
        ...
        sample_true_liquids, sample_true_concentrations = self._split_component_arrays(
            y_sample_true,
            idx_to_joint_label,
        )
        sample_pred_liquids, sample_pred_concentrations = self._split_component_arrays(
            y_sample_pred,
            idx_to_joint_label,
        )
        sample_liquid_acc = float((sample_true_liquids == sample_pred_liquids).mean())
        sample_concentration_acc = float((sample_true_concentrations == sample_pred_concentrations).mean())
        measurement_confusion_matrix = compute_confusion_matrix(
            y_test,
            y_measurement_pred,
            num_classes=len(idx_to_joint_label),
        )
    ...
    def fit(self, split: RepeatedMeasurementSplit) -> GroupedJointClassificationFoldResult:
        ...
        y_test = np.asarray([joint_label_to_idx[label] for label in test_joint_labels], dtype=np.int64)

        train_loader = DataLoader(
            SignalDataset(split.x_train, y_train),
            batch_size=self.config.batch_size,
            shuffle=True,
            num_workers=self.config.num_workers,
        )

        model = GroupedJointClassificationCNN1D(
        ...
        for epoch_idx in range(self.config.epochs):
            ...
            history["train_joint_acc"].append(train_acc)
            ...
            if self.config.verbose:
                ...
                print(
                    ...
                    f"train_joint_acc={train_acc:.4f} "
        ...
        return GroupedJointClassificationFoldResult(
            fold=split.fold,
            model=model,
            history=history,
            measurement_joint_acc=measurement_joint_acc,
