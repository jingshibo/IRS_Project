...
def summarize_grouped_joint_classification_results(fold_results: List[object]) -> Dict[str, object]:
...
def print_grouped_joint_classification_summary(summary: Dict[str, object]) -> None:
    print(
        f"Grouped joint classification measurement joint acc={summary['mean_measurement_joint_acc']:.4f} "
        f"+/- {summary['std_measurement_joint_acc']:.4f}"
    )
    print(
        f"Grouped joint classification sample joint acc={summary['mean_sample_joint_acc']:.4f} "
        f"+/- {summary['std_sample_joint_acc']:.4f}"
    )
    print(
        f"Grouped joint classification measurement liquid acc={summary['mean_measurement_liquid_acc']:.4f} "
        f"+/- {summary['std_measurement_liquid_acc']:.4f}"
    )
    print(
        f"Grouped joint classification sample liquid acc={summary['mean_sample_liquid_acc']:.4f} "
        f"+/- {summary['std_sample_liquid_acc']:.4f}"
    )
    print(
        f"Grouped joint classification measurement concentration-group acc="
        f"{summary['mean_measurement_concentration_acc']:.4f} "
        f"+/- {summary['std_measurement_concentration_acc']:.4f}"
    )
    print(
        f"Grouped joint classification sample concentration-group acc="
        f"{summary['mean_sample_concentration_acc']:.4f} "
...
def plot_grouped_recall_matrix_with_numbers(
    ...
    *,
    ...
    ax.set_title(title or "Grouped Joint Classification Recall Matrix")
