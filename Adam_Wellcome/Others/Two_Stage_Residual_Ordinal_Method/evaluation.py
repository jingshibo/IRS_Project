...
def summarize_two_stage_residual_ordinal_results(fold_results: List[object]) -> Dict[str, object]:
...
def print_two_stage_residual_ordinal_summary(summary: Dict[str, object]) -> None:
    print(
        f"Two-stage residual ordinal measurement joint acc={summary['mean_measurement_joint_acc']:.4f} "
        f"+/- {summary['std_measurement_joint_acc']:.4f}"
    )
    print(
        f"Two-stage residual ordinal sample joint acc={summary['mean_sample_joint_acc']:.4f} "
    ...
    print(
        f"Stage-2 measurement concentration acc, conditional on correct liquid={summary['mean_measurement_concentration_acc']:.4f} "
        f"+/- {summary['std_measurement_concentration_acc']:.4f}"
    )
    print(
        f"Stage-2 sample concentration acc, conditional on correct liquid={summary['mean_sample_concentration_acc']:.4f} "
...
def plot_grouped_recall_matrix(
    ...
    *,
    ...
    ax.set_title(title or f"Two-Stage Residual Ordinal {matrix_name.title()} Recall Matrix")
