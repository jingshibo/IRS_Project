...
def _build_model(model_name: str, in_channels: int, num_classes: int, params: Dict[str, Any]) -> nn.Module:
    ...
    raise ValueError(
        f"Unsupported model_name='{model_name}'. Use 'shared_backbone_2ch', 'two_tower_late_fusion', 'two_tower_mid_fusion_cnn'."
    )
...
def run_grid_search_three_models(
    ...
    model_names: Sequence[str] = ("shared_backbone_2ch","two_tower_late_fusion", "two_tower_mid_fusion_cnn"),
    ...
    """Grid search across three model families on existing cv_folds.
...
if __name__ == "__main__":
    print("Utility module for grid search across three CNN model families.")
    print("Import and call `run_grid_search_three_models(cv_folds, ...)` from your notebook/script.")
    print(f"Default search-space jobs across 3 models: {estimate_total_trials(default_search_space(), ('shared_backbone_2ch', 'two_tower_late_fusion', 'two_tower_mid_fusion_cnn'))}")
